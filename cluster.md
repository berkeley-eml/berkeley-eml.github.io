...
servers (aka 'submit hosts'):
    ...
    blundell, jorgensen, laffont, logit, radner, theil

Users can also start [JupyterHub sessions](jupyterhub.md) to get
...
|-------- |-------------- |--------- |---------------- |------------- |
|high (default)|352|28 days[^time-limit]|132 GB (eml-sm2 nodes) or 768 GB (eml-sm3 nodes, requested with "-C mem768g" [formerly "-C bigmem"] [^large-mem])|56 (eml-sm2 nodes) or 32 (eml-sm3 nodes)[^max-cores]|
|low[^low-partition]|256|28 days[^time-limit]|264 GB|32[^max-cores]|
|gpu[^gpu-partition]|48|28 days|256 GB|48|

[^low-partition]: See [How to Submit Jobs to the Low Partition](#low).

[^time-limit]: See [Submitting Long Jobs](#submitting-long-jobs-and-setting-job-time-limits) for jobs you expect to take more than three days.

[^max-cores]: If you use software set up to use multiple nodes, you can run individual jobs on more than 56 (or 32 on the low partition) cores. See [Submitting Multi-node Jobs](#submitting-multi-node-jobs) or [Submitting MPI Jobs](#submitting-mpi-jobs) or [Submitting MATLAB Parallel Server Jobs](#submitting-matlab-parallel-server-multi-node-jobs).

[^large-mem]: See [Submitting Large Memory Jobs](#submitting-large-memory-jobs) for jobs needing more than 25 GB memory.

[^gpu-partition]: See [Submitting GPU Jobs](#submitting-gpu-jobs) for jobs using the GPU.
...
above and discussed [further here](memory.md).
...
To use PyTorch with the GPU, you just need to run `import torch`.
At the moment, one can't directly use Tensorflow with the GPU, but
if you need this, please contact us as we can fix that or give a work-around (basically creating a Conda environment using Python 3.12 and then `pip install tensorflow[and-cuda]`.
...
order to assess CPU or memory use, you can SSH to the node in
the context of your existing job. (Otherwise SSH to a cluster node will
fail if you don't have a job running on the node.) You'll need to
determine the node on which your job is running and then ssh to that node
from a standalone/login node. Note that if your terminal on the standalone/login node is via JupyterHub, you'll need this SSH invocation: `ssh -F none <name_of_node>`.
...
#### Option 1 - submit a Slurm job via sbatch that uses MATLAB Parallel Server (new approach for MATLAB R2023a, as well as R2022a)
...
(`dcsProfile.settings` in MATLAB R2022a) and click 'Open'. If it imports
under the name `dcs_Import` rather than `dcs`, rename it to `dcs`.
Then exit out of the Cluster Profile Manager.
